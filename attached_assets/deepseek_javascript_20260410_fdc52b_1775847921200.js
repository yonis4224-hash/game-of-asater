...
io.on('connection', (socket) => {
    ...
    function startGame(roomCode) {
        ...
        console.log(`🎮 بدء اللعبة في ${roomCode} - وضع ${room.gameMode}`);
        room.currentRound = 1;
    ...
    function loadRound1v1(roomCode, round) {
        ...
        room.roundData = {
            type: type,
            questions: questionsList.slice(0, 5), // 5 أسئلة فقط
            currentIndex: 0,
    ...
    socket.on('submitAnswer1v1', ({ roomCode, answer }) => {
        ...
        if (room.roundData.answers.teamA !== null && room.roundData.answers.teamB !== null) {
            // الانتقال إلى السؤال التالي بعد 2 ثانية
            ...
            setTimeout(() => {
                ...
                if (room.roundData.currentIndex >= room.roundData.questions.length) {
                    ...
                    // الانتقال إلى الجولة التالية بعد 4 ثوانٍ
                    setTimeout(() => {
    ...
    // ==================== نمط 2 ضد 2 ====================
    // (مبسط هنا للاختصار، لكن يمكنك إكماله بنفس منطق 1v1)
    function loadRound2v2(roomCode, round) {
        const room = rooms.get(roomCode);
        if (!room) return;
        // سيتم إكماله لاحقاً
        io.to(roomCode).emit('message', 'جاري تطوير نمط 2 ضد 2...');
    }
